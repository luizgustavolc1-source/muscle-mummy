...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { type, client } = await request.json();
    if (!["workout", "nutrition"].includes(type) || !client?.full_name) {
